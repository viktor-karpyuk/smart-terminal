...
class TeamsService {
  ...
  connection() {
    ...
    const hasGraph = Boolean(
      this.store.setting('graph.tenantId') && this.store.setting('graph.clientId') && this.store.secret('graph.clientSecret'),
    );
    return {
      way,
  ...
  saveConnection({ way, webhookUrl, tenantId, clientId, clientSecret } = {}) {
  ...
  async deliver(message, person) {
    ...
    const credentials = {
      ...
      botUserId: this.store.setting('graph.botUserId') || null,
    };
