...
function registerIpc() {
  ...
  ipcMain.handle('workspace:load', (event) => {
    ...
    return {
      ...
      minimized,
      // The tabs that are not sessions: folder tabs, the monitor, the extensions
      // gallery, a panel an extension brought. The layout names them, nothing
      // else does, and a tab whose panel is missing is pruned as a dead session.
      panels: stored.panels ?? [],
      // And the sections set aside whole, whose tabs are in neither the layout
      // nor the dock.
  ...
  ipcMain.on('workspace:save', (event, state) => {
    ...
    db.saveWorkspace(windowId, {
      ...
      // Sent by the renderer since the day panels existed, and dropped here
      // until now: a folder tab, the monitor and a panel an extension brought
      // are tabs with no session behind them, so an id on its own restores
      // nothing.
      panels: state.panels ?? [],
  ...
  const KUBE = {
    ...
    configure: (args) => kube.configure(args),
